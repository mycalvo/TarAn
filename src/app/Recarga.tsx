...
// Clave por defecto si no se edita manualmente
const DEFAULT_KEY_B = "6C8B8827F346";
...
export default function Recarga({ onBack }: RecargaProps) {
  ...
  const [customKeyB, setCustomKeyB] = useState<string>(DEFAULT_KEY_B); // Estado para la Key B
  ...
  const [lastErrorLog, setLastErrorLog] = useState<string | null>(null); // Log explícito de error
  ...
  // Helper: Transforma hex string a array de bytes
  ...
  const handleInyeccion = async () => {
    ...
    // Validar formato de Key B manual (12 caracteres hexadecimales = 6 bytes)
    const cleanKeyB = customKeyB.replace(/\s+/g, "").toUpperCase();
    if (cleanKeyB.length !== 12 || !/^[0-9A-F]{12}$/.test(cleanKeyB)) {
      Alert.alert(
        "Error de Clave",
        "La Key B debe tener exactamente 12 caracteres hexadecimales (ej. 6C8B8827F346).",
      );
      return;
    }
    ...
    try {
      ...
      // 2. Autenticar Sector 9 con Key B ingresada
      const keyBBytes = hexStringToHexArray(cleanKeyB);
      ...
      console.warn("Error durante la escritura NFC:", ex);

      // Extracción del mensaje exacto retornado por el sistema/NFC driver
      const rawError = ex?.message || JSON.stringify(ex) || "Error desconocido";
      let interpretation = "Error de comunicación o desconexión física de la tarjeta.";

      if (
        rawError.includes("Tag response error") ||
        rawError.includes("transceive failed") ||
        rawError.includes("Tech is not supported")
      ) {
        interpretation =
          "HARDWARE INCOMPATIBLE: Tu smartphone carece del chip NXP necesario para escribir en tarjetas MIFARE Classic.";
      } else if (
        rawError.includes("Authentication failed") ||
        rawError.includes("Auth failed")
      ) {
        interpretation =
          "CLAVE INCORRECTA: La Key B especificada no fue aceptada para el Sector 9.";
      } else if (rawError.includes("Write failed")) {
        interpretation =
          "ESCRITURA DENEGADA: Autenticado correctamente, pero los Access Bits de la tarjeta bloquean la escritura en los bloques 37/38.";
      }

      const fullLog = `MOTIVO: ${rawError}\nINTERPRETACIÓN: ${interpretation}`;
      setLastErrorLog(fullLog);

      Alert.alert("Error de Inyección", fullLog);
      setStatus("IDLE");
    } finally {
      NfcManager.cancelTechnologyRequest().catch(() => 0);
    }
      ...
      {status === "IDLE" || status === "PROCESSING" ? (
        <View style={styles.body}>
          <View style={styles.cardBox}>
            ...
            {/* Input manual de Key B para pruebas */}
            <View style={styles.inputWrapper}>
              <Text style={styles.inputLabel}>
                &gt; CLAVE DE ESCRITURA (KEY B HEX - 6 BYTES):
              </Text>
              <TextInput
                style={styles.keyInput}
                value={customKeyB}
                onChangeText={(text) => setCustomKeyB(text.toUpperCase())}
                maxLength={12}
                autoCapitalize="characters"
                placeholder="6C8B8827F346"
                editable={status !== "PROCESSING"}
                placeholderTextColor="rgba(255, 85, 0, 0.3)"
              />
            </View>
          ...
          {/* Visor de Error en tiempo real */}
          {lastErrorLog && (
            <View style={styles.errorBox}>
              <Text style={styles.errorTitle}>
                &gt; DETALLE DEL DIAGNÓSTICO DE ERROR:
              </Text>
              <Text style={styles.errorText}>{lastErrorLog}</Text>
            </View>
          )}
          ...
          <View style={styles.diagnosticsBox}>
            ...
            <View style={styles.diagBadge}>
              <Text style={styles.diagBadgeTxt}>
                KEY B ACTIVA: {customKeyB.toUpperCase() || "NO DEFINIDA"}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.successContainer}>
          ...
          <View style={styles.hexBlobBox}>
            ...
            <Text style={styles.hexBlobDesc}>
              • Auth: Key B ({customKeyB}) verificada.
            </Text>
          </View>
